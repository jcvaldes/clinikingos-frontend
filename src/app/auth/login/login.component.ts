...
export class LoginComponent implements OnInit, OnDestroy {
  ...
  googleInit() {
    gapi.load('auth2', () => {
      this.auth2 = gapi.auth2.init({
        client_id: '395109064035-5kl6amnhpt5tkabfkisqg07kkrtrj436.apps.googleusercontent.com',
  ...
  ingresar(f: NgForm) {
    ...
    // this._authService.login( user, f.value.rememberme )
    //     .subscribe((data: any) => {
    //       this.store.dispatch(new DesactivarLoadingAction());
    //       this._userService.saveStorage(
    //           data.id,
    //           data.token,
    //           data.user,
    //           data.menu
